...
const DeletedLogs = () => {
  ...
  const handleRestoreLog = async (log) => {
    if (
      !window.confirm(
        `Are you sure you want to restore this ${log.entityType}?`
      )
    )
      return;
  ...
  return (
    <div
      className="relative container mx-auto p-6 bg-gray-100 min-h-screen"
      aria-busy={isLoading}
    >
      <div
      ...
      {isModalOpen && selectedLog && (
        <div className="fixed inset-0 bg-black/90 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-4 sm:p-6 w-[95%] sm:max-w-md max-h-[90vh] overflow-y-auto no-scrollbar">
            ...
            <div>
              <p className="mb-2">
                <strong>Details:</strong> {getDetails(selectedLog)}
              </p>
              <p className="mb-2">
                <strong>Deleted From:</strong>{" "}
                {getDeletedFrom(selectedLog.entityType)}
              </p>
              <p className="mb-2">
                <strong>Deleted By:</strong> {selectedLog.deletedBy.name} (
                {selectedLog.deletedBy.email})
              </p>
              <p className="mb-2">
                <strong>Deleted At:</strong>{" "}
                {formatTimestamp(selectedLog.deletedAt)}
              </p>
              <p className="mb-2">
              ...
              {selectedLog.entityType === "User" ? (
                <div className="ml-4 space-y-2">
                  <p>
                    <strong>Name:</strong>{" "}
                    {selectedLog.entityDetails.name || "N/A"}
                  </p>
                  <p>
                    <strong>Email:</strong>{" "}
                    {selectedLog.entityDetails.email || "N/A"}
                  </p>
                  <p>
                    <strong>Roles:</strong>{" "}
                  ...
                  <p>
                    <strong>Phone:</strong>{" "}
                    {selectedLog.entityDetails.phone || "N/A"}
                  </p>
                  <p>
                    <strong>Block:</strong>{" "}
                ...
                <div className="ml-4 space-y-2">
                  <p>
                    ...
                    {selectedLog.entityDetails.userName || "N/A"}
                  </p>
                  <p>
                    <strong>Description:</strong>{" "}
                    {selectedLog.entityDetails.description || "N/A"}
                  </p>
                  <p>
                    <strong>Status:</strong>{" "}
                    {selectedLog.entityDetails.status || "N/A"}
                  </p>
                  <p>
                    <strong>Priority:</strong>{" "}
                    {selectedLog.entityDetails.priority || "N/A"}
                  </p>
                  <p>
                    <strong>Branch:</strong>{" "}
                    {selectedLog.entityDetails.branch
                  ...
                  <p>
                    ...
                    {selectedLog.entityDetails.attachment ? "Yes" : "None"}
                  </p>
                  <p>
                    <strong>Rating:</strong>{" "}
                    {selectedLog.entityDetails.rating || "N/A"}
                  </p>
                  <p>
                    <strong>Created At:</strong>{" "}
                    {formatTimestamp(selectedLog.entityDetails.createdAt)}
                  </p>
                  <p>
                    <strong>Updated At:</strong>{" "}
                    {formatTimestamp(selectedLog.entityDetails.updatedAt)}
                  </p>
                </div>
                ...
                <div className="ml-4 space-y-2">
                  <p>
                    <strong>Code:</strong>{" "}
                    {selectedLog.entityDetails.branchCode || "N/A"}
                  </p>
                  <p>
                    <strong>Name:</strong>{" "}
                    {selectedLog.entityDetails.branchName || "N/A"}
                  </p>
                  <p>
                    <strong>Created At:</strong>{" "}
                    {formatTimestamp(selectedLog.entityDetails.createdAt)}
                ...
                <div className="ml-4 space-y-2">
                  <p>
                    <strong>Code:</strong>{" "}
                    {selectedLog.entityDetails.departmentCode || "N/A"}
                  </p>
                  <p>
                    <strong>Name:</strong>{" "}
                    {selectedLog.entityDetails.departmentName || "N/A"}
                  </p>
                  <p>
                    <strong>Created At:</strong>{" "}
                    {formatTimestamp(selectedLog.entityDetails.createdAt)}
                ...
                <div className="ml-4 space-y-2">
                  <p>
                    <strong>Code:</strong>{" "}
                    {selectedLog.entityDetails.blockCode || "N/A"}
                  </p>
                  <p>
                    <strong>Name:</strong>{" "}
                    {selectedLog.entityDetails.blockName || "N/A"}
                  </p>
                  <p>
                    <strong>Created At:</strong>{" "}
                    {formatTimestamp(selectedLog.entityDetails.createdAt)}
                ...
                <div className="ml-4 space-y-2">
                  <p>
                    ...
                    {JSON.stringify(selectedLog.entityDetails)}
                  </p>
                </div>
              )}
              <p className="mb-2">
                <strong>Reason:</strong> {selectedLog.reason || "N/A"}
              </p>
