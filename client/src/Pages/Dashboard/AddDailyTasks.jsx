...
const emptyTask = () => ({
  ...
  project: "",
  activity: "",
  hours: "", // whole hours string
  minutes: "0", // minutes string 0-59
  description: "",
});

// Convert hours + minutes → decimal hours for API (e.g. 1h 30m → 1.5)
...
// Display total as "Xh Ym"
const formatTotal = (tasks) => {
  const totalMins = tasks.reduce((sum, t) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const validTasks = tasks.filter(
      (t) =>
        t.project &&
        t.activity &&
        (parseFloat(t.hours) > 0 || parseFloat(t.minutes) > 0) &&
        t.description.trim()
    );
    ...
    try {
      ...
      const payload = {
        ...
        tasks: validTasks.map(
          ({ project, activity, hours, minutes, description }) => ({
            project,
            activity,
            hours: toDecimalHours(hours, minutes), // stored as decimal in DB
            description,
          })
        ),
      };
              ...
              {tasks.map((task, index) => (
                ...
                >
                  ...
                  <div className="grid grid-cols-4 gap-4 mb-3">
                    ...
                    <div>
                      ...
                      <div className="relative">
                        <input
                          type="number"
                          className="w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-primary bg-white pr-10"
                          value={task.hours}
                          onChange={(e) =>
                            updateTask(task.id, "hours", e.target.value)
                          }
                    ...
                    <div>
                      ...
                      <div className="relative">
                        <select
                          ...
                          value={task.minutes}
                          onChange={(e) =>
                            updateTask(task.id, "minutes", e.target.value)
                          }
                        >
                  ...
                  {(parseFloat(task.hours) > 0 ||
                    ...
                    <div className="mb-3">
                      <span className="text-xs bg-orange-50 text-orange-600 border border-orange-200 px-2 py-1 rounded-md">
                        ⏱ {parseFloat(task.hours) > 0 ? `${task.hours}h ` : ""}
                        {parseFloat(task.minutes) > 0
                          ? `${task.minutes}m`
                          : ""}{" "}
                        = {toDecimalHours(task.hours, task.minutes).toFixed(2)}{" "}
                        decimal hrs
                      </span>
                    </div>
                  )}
