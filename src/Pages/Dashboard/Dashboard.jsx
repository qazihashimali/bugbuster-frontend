...
// // import { FaPerson, FaScaleBalanced, FaClock, FaFileInvoice, FaStar, FaStarHalfStroke } from "react-icons/fa6";
...
// //   const [formData, setFormData] = useState({ status: "", rating: 0 });
...
// //           const isCreator = issue.createdBy?._id === userId;
// //           const isAssignee = issue.assignedTo?._id === userId;
// //           console.log(
...
// //     setFormData({ status: issue.status || "", rating: issue.rating || 0 });
// //     setIsModalOpen(true);
// //   };
...
// //       if (formData.rating !== selectedIssue.rating) payload.rating = formData.rating;
...
// //                     const isAssignee =
// //                       selectedIssue.assignedTo &&
// //                       selectedIssue.assignedTo._id === user._id;
// //                     const isCreator =
// //                       selectedIssue.createdBy &&
// //                       selectedIssue.createdBy._id === user._id;
// //                     const canUpdateStatus = isAdmin || (isServiceProvider && isAssignee);
// //                     const canUpdateRating = isAdmin || isCreator;
...
// //                           </div>
// //                         )}
// //                         {(canUpdateStatus || canUpdateRating) ? (
...
//       // Check if status is "resolved" and rating is 2.5 or below
//       const effectiveStatus = payload.status || selectedIssue.status;
//       const effectiveRating = payload.rating !== undefined ? payload.rating : currentRating;
//       if (effectiveStatus === "resolved" && effectiveRating <= 2.5) {
...
//       const token = localStorage.getItem("token");
//       const currentRating = selectedIssue.rating ?? 0;
//       const newRating = formData.rating !== currentRating ? formData.rating : currentRating;
//       const payload = {
//         status: reopen ? "pending" : "resolved",
//         rating: newRating,
//       };
...
//       showAlert(
//         "success",
//         reopen ? "Task reopened successfully!" : "Task updated successfully!"
//       );
//     } catch (err) {
//       console.error("Update issue error:", err);
//       showAlert("error", err.message || "Failed to update task");
...
//                 {user ? (
//                   (() => {
//                     const isSuperAdminOrAdmin = user.roles && (user.roles.includes("SuperAdmin") || user.roles.includes("Admin"));
//                     const isAssignee =
//                       selectedIssue.assignedTo &&
...
//                       (formData.status === "resolved" || selectedIssue.status === "resolved") &&
//                       (isSuperAdminOrAdmin || isCreator);
...
//           message="Do you want to reopen this task?"
//         />
